var MockerRegistry = class {
    ...
    register(typeOrEvent, raw, id, url, factoryOrRedirect) {
      let type = 'object' == typeof typeOrEvent ? typeOrEvent.type : typeOrEvent
      if ('object' == typeof typeOrEvent) {
      ...
      if ('manual' === type) {
        if ('function' != typeof factoryOrRedirect)
          throw new TypeError(
      ...
      if ('redirect' === type) {
        if ('string' != typeof factoryOrRedirect)
          throw new TypeError(
...
function mockObject(options, object, mockExports = {}) {
  let finalizers = new Array(),
    ...
    mockPropertiesOf = (container, newContainer) => {
      let containerType = getType(container),
        isModule = 'Module' === containerType || !!container.__esModule
      for (let { key: property, descriptor } of getAllMockableProperties(
        container,
        isModule,
        ...
        let isFunction = type.includes('Function') && 'function' == typeof value
        if (
          ...
          if (define(newContainer, property, isFunction ? value : {})) {
            if (isFunction) {
              let mockFunction = function () {
                if (this instanceof newContainer[property])
                  for (let {
                    ...
                    let value2 = this[key]
                    if (
                      getType(value2).includes('Function') &&
                      'function' == typeof value2
                    ) {
                      let original = this[key],
                        mock2 = spyOn(this, key).mockImplementation(original),
                        origMockReset = mock2.mockReset
              ...
              let spyOn = options.spyOn,
                mock = spyOn(newContainer, property)
              if ('automock' === options.type) {
              ...
              Object.defineProperty(newContainer[property], 'length', {
                value: 0,
              })
            }
        ...
        } else define(newContainer, property, value)
      }
    },
...
function getAllMockableProperties(obj, isModule, constructors) {
  ...
  do {
    if (
      curr === Object2.prototype ||
      curr === Function2.prototype ||
      curr === RegExp2.prototype
    )
      break
    collectOwnProperties(curr, (key) => {
      let descriptor = Object2.getOwnPropertyDescriptor(curr, key)
      descriptor && allProps.set(key, { key, descriptor })
    })
  } while ((curr = Object2.getPrototypeOf(curr)))
  if (isModule && !allProps.has('default') && 'default' in obj) {
    let descriptor = Object2.getOwnPropertyDescriptor(obj, 'default')
    descriptor && allProps.set('default', { key: 'default', descriptor })
  }
  return Array2.from(allProps.values())
}
function collectOwnProperties(obj, collector) {
  let collect =
    'function' == typeof collector ? collector : (key) => collector.add(key)
...
var _UNC_REGEX = /^[/\\]{2}/,
  ...
  normalize = function (path) {
    ...
    let isUNCPath = (path = normalizeWindowsPath(path)).match(_UNC_REGEX),
      isPathAbsolute = isAbsolute(path),
      trailingSeparator = '/' === path[path.length - 1]
    return 0 === (path = normalizeString(path, !isPathAbsolute)).length
      ? isPathAbsolute
        ? '/'
        : trailingSeparator
          ? './'
      ...
      : (trailingSeparator && (path += '/'),
        _DRIVE_LETTER_RE.test(path) && (path += '/'),
        isUNCPath
          ? isPathAbsolute
            ? `//${path}`
            : `//./${path}`
  ...
  join = function (...segments) {
    ...
    for (let seg of segments)
      if (seg)
        if (path.length > 0) {
          let pathTrailing = '/' === path[path.length - 1],
            segLeading = '/' === seg[0]
          path +=
            pathTrailing && segLeading
              ? seg.slice(1)
              : pathTrailing || segLeading
                ? seg
                : `/${seg}`
    ...
    if ('/' === char) {
      if (lastSlash !== index2 - 1 && 1 !== dots)
        if (2 === dots) {
          ...
          allowAboveRoot &&
            ((res += res.length > 0 ? '/..' : '..'), (lastSegmentLength = 2))
        } else
          (res.length > 0
...
function C(n = !1) {
  let e = typeof process < 'u' ? process : void 0,
    ...
    g = e?.argv || []
  return (
    (!('NO_COLOR' in i || g.includes('--no-color')) &&
      ('FORCE_COLOR' in i ||
        g.includes('--color') ||
        'win32' === e?.platform ||
        (n && 'dumb' !== i.TERM) ||
...
var hasRequiredReactIs_production_min,
  hasRequiredReactIs,
  reactIsExports$1 = requireReactIs$1(),
  index$1 = getDefaultExportFromCjs(reactIsExports$1),
  ReactIs19 = _mergeNamespaces({ __proto__: null, default: index$1 }, [
    reactIsExports$1,
  ]),
  reactIs = { exports: {} },
...
var reactIsExports = requireReactIs(),
  index = getDefaultExportFromCjs(reactIsExports),
  ReactIs18 = _mergeNamespaces({ __proto__: null, default: index }, [
    reactIsExports,
  ]),
  reactIsMethods = [
    'isAsyncMode',
  ...
  Array.isArray(getPromiseDetails(Promise.resolve())) &&
    (getPromiseValue = (value, options) => {
      ...
      return state === kPending
        ? 'Promise{<pending>}'
        : `Promise${state === kRejected ? '!' : ''}{${options.inspect(innerValue, options)}}`
    })
} catch {}
function createSimpleStackTrace(options) {
  let { message = '$$stack trace error', stackTraceLimit = 1 } = options || {},
...
function requireJsTokens() {
  return hasRequiredJsTokens
    ...
    : ((hasRequiredJsTokens = 1),
      ...
      (jsTokens_1 = function* (input, { jsx = !1 } = {}) {
        var braces,
          firstCodePoint,
          isExpression,
          lastIndex,
        ...
        for (
          ...
          switch ((mode = stack[stack.length - 1]).tag) {
            case 'JS':
            ...
            case 'InterpolationInJSX':
              ...
              if (
                ...
                switch (
                  ...
                  case '(':
                    ;('?NonExpressionParenKeyword' === lastSignificantToken &&
                      stack.push({
                  ...
                  case '{':
                    ;((Punctuator.lastIndex = 0),
                      (isExpression =
                        !TokensNotPrecedingObjectLiteral.test(
                          lastSignificantToken,
                        ) &&
                        (TokensPrecedingExpression.test(lastSignificantToken) ||
                          KeywordsWithExpressionAfter.test(
                            lastSignificantToken,
                          ))),
                  ...
                  case '}':
                    ...
                    nextLastSignificantToken = (postfixIncDec = braces.pop())
                      ? '?ExpressionBraceEnd'
                      : '}'
                    break
                  case ']':
                  ...
                  case '<':
                    if (
                      ...
                      ;(stack.push({ tag: 'JSXTag' }),
                        (lastIndex += 1),
                        (lastSignificantToken = '<'),
                        yield { type: 'JSXPunctuator', value: punctuator })
                      continue
                    }
                    postfixIncDec = !1
                    break
                ...
                ;((lastIndex = nextLastIndex),
                  ...
                  yield { type: 'Punctuator', value: punctuator })
                continue
              }
              if (
                ((Identifier.lastIndex = lastIndex),
                (match = Identifier.exec(input)))
              ) {
                switch (
                ...
                ;((lastSignificantToken = nextLastSignificantToken),
                  ...
                  yield {
                    type:
                      '#' === match[1] ? 'PrivateIdentifier' : 'IdentifierName',
                    value: match[0],
                  })
                continue
              }
              if (
                ...
                (match = StringLiteral.exec(input)))
              ) {
              ...
              if (
                ...
                (match = NumericLiteral.exec(input)))
              ) {
                ;((lastIndex = NumericLiteral.lastIndex),
                  (lastSignificantToken = match[0]),
                  (postfixIncDec = !0),
                  yield { type: 'NumericLiteral', value: match[0] })
                continue
              }
              if (
                ((Template.lastIndex = lastIndex),
                ...
                ;((lastIndex = Template.lastIndex),
                  ...
                  '${' === match[1]
                    ? ((lastSignificantToken = '?InterpolationInTemplate'),
                      ...
                      (postfixIncDec = !1),
                      yield { type: 'TemplateHead', value: match[0] })
                    : ((postfixIncDec = !0),
                      yield {
            ...
            case 'JSXTagEnd':
              if (
                ...
                (match = JSXPunctuator.exec(input)))
              ) {
                switch (
                  ...
                  case '>':
                    ;(stack.pop(),
                      '/' === lastSignificantToken || 'JSXTagEnd' === mode.tag
                  ...
                  case '/':
                    '<' === lastSignificantToken &&
                      (stack.pop(),
                      'JSXChildren' === stack[stack.length - 1].tag &&
                        stack.pop(),
                      stack.push({ tag: 'JSXTagEnd' }))
                }
                ;((lastSignificantToken = nextLastSignificantToken),
                  yield { type: 'JSXPunctuator', value: match[0] })
                continue
              }
              if (
                ((JSXIdentifier.lastIndex = lastIndex),
                (match = JSXIdentifier.exec(input)))
              ) {
                ;((lastIndex = JSXIdentifier.lastIndex),
                  (lastSignificantToken = match[0]),
                  yield { type: 'JSXIdentifier', value: match[0] })
                continue
              }
              if (
                ((JSXString.lastIndex = lastIndex),
                (match = JSXString.exec(input)))
              ) {
            ...
            case 'JSXChildren':
              if (
                ((JSXText.lastIndex = lastIndex), (match = JSXText.exec(input)))
              ) {
                ;((lastIndex = JSXText.lastIndex),
                  (lastSignificantToken = match[0]),
                  yield { type: 'JSXText', value: match[0] })
                continue
              }
          ...
          (match = WhiteSpace.exec(input)))
            ? ((lastIndex = WhiteSpace.lastIndex),
              yield { type: 'WhiteSpace', value: match[0] })
            : ((LineTerminatorSequence.lastIndex = lastIndex),
              (match = LineTerminatorSequence.exec(input))
                ? ((lastIndex = LineTerminatorSequence.lastIndex),
                  (postfixIncDec = !1),
                  KeywordsWithNoLineTerminatorAfter.test(
                    lastSignificantToken,
                  ) && (lastSignificantToken = '?NoLineTerminatorHere'),
                  yield { type: 'LineTerminatorSequence', value: match[0] })
                : ((MultiLineComment.lastIndex = lastIndex),
                  (match = MultiLineComment.exec(input))
                    ? ((lastIndex = MultiLineComment.lastIndex),
                      Newline.test(match[0]) &&
                        ((postfixIncDec = !1),
                        KeywordsWithNoLineTerminatorAfter.test(
                          lastSignificantToken,
                        ) && (lastSignificantToken = '?NoLineTerminatorHere')),
                    ...
                    : ((SingleLineComment.lastIndex = lastIndex),
                      (match = SingleLineComment.exec(input))
                        ? ((lastIndex = SingleLineComment.lastIndex),
                          (postfixIncDec = !1),
                          yield { type: 'SingleLineComment', value: match[0] })
                        : ((lastIndex += (firstCodePoint = String.fromCodePoint(
                            input.codePointAt(lastIndex),
                          )).length),
                          (lastSignificantToken = firstCodePoint),
                          (postfixIncDec = !1),
                          yield {
                            type: mode.tag.startsWith('JSX')
                              ? 'JSXInvalid'
                              : 'Invalid',
...
function normalizeWindowsPath2(input = '') {
  return (
    ...
    input
      .replace(/\\/g, '/')
      .replace(_DRIVE_LETTER_START_RE2, (r) => r.toUpperCase())
  )
}
var _IS_ABSOLUTE_RE2 = /^[/\\](?![/\\])|^[/\\]{2}(?!\.)|^[A-Za-z]:[/\\]/
function cwd() {
  return typeof process < 'u' && 'function' == typeof process.cwd
...
var resolve = function (...arguments_) {
  ...
  for (
    let index2 =
      (arguments_ = arguments_.map((argument) =>
        normalizeWindowsPath2(argument),
      )).length - 1;
    index2 >= -1 && !resolvedAbsolute;
    index2--
  ) {
  ...
  return (
    ...
    resolvedAbsolute && !isAbsolute2(resolvedPath)
      ? `/${resolvedPath}`
...
function extractLocation(urlLike) {
  ...
  let url = parts[1]
  if (
    (url.startsWith('async ') && (url = url.slice(6)),
...
function parseSingleFFOrSafariStack(raw) {
  let line = raw.trim()
  if (
    SAFARI_NATIVE_CODE_REGEXP.test(line) ||
    (line.includes(' > eval') &&
      (line = line.replace(
  ...
  let functionNameRegex = /((.*".+"[^@]*)?[^@]*)(@)/,
    ...
    [url, lineNumber, columnNumber] = extractLocation(
...
function parseSingleStack(raw) {
  let line = raw.trim()
  return CHROME_IE_STACK_REGEXP.test(line)
    ? parseSingleV8Stack(line)
    : parseSingleFFOrSafariStack(line)
}
function parseSingleV8Stack(raw) {
  ...
  let sanitizedLine = line
      ...
      .replace(/\(eval code/g, '(')
      .replace(/^.*?\s+/, ''),
    location2 = sanitizedLine.match(/ (\(.+\)$)/)
  sanitizedLine = location2
    ? sanitizedLine.replace(location2[0], '')
    : sanitizedLine
  let [url, lineNumber, columnNumber] = extractLocation(
      location2 ? location2[1] : sanitizedLine,
    ),
    method = (location2 && sanitizedLine) || '',
    file = url && ['eval', '<anonymous>'].includes(url) ? void 0 : url
  return file && lineNumber && columnNumber
    ? (method.startsWith('async ') && (method = method.slice(6)),
      file.startsWith('file://') && (file = file.slice(7)),
      (file =
        file.startsWith('node:') || file.startsWith('internal:')
          ? file
          : resolve(file)),
      method && (method = method.replace(/__vite_ssr_import_\d+__\./g, '')),
      {
        method,
        file,
        line: Number.parseInt(lineNumber),
...
function createCompilerHints(options) {
  ...
  function _mocker() {
    return typeof globalThis[globalThisAccessor] < 'u'
  ...
  return {
    ...
    importActual: async (path) =>
      _mocker().importActual(path, getImporter('importActual')),
    importMock: async (path) =>
      _mocker().importMock(path, getImporter('importMock')),
  }
}
function getImporter(name) {
  let stackArray = createSimpleStackTrace({ stackTraceLimit: 5 }).split('\n'),
    importerStackIndex = stackArray.findIndex(
      (stack2) =>
        stack2.includes(` at Object.${name}`) || stack2.includes(`${name}@`),
    ),
    stack = parseSingleStack(stackArray[importerStackIndex + 1])
  return stack?.file || ''
}
var hot = import.meta.hot || { on: warn, off: warn, send: warn }
function warn() {
  console.warn(
    "Vitest mocker cannot work if Vite didn't establish WS connection.",
  )
}
var { now } = Date,
  ModuleMocker = class {
    ...
    async importActual(id, importer) {
      ...
      let ext = extname(resolved.id),
        url = new URL(resolved.url, location.href),
        query = `_vitest_original&ext${ext}`,
        actualUrl = `${url.pathname}${url.search ? `${url.search}&${query}` : `?${query}`}${url.hash}`
      return this.wrapDynamicImport(() => import(actualUrl)).then((mod) => {
    ...
    async importMock(rawId, importer) {
      await this.prepare()
      let { resolvedId, resolvedUrl, redirectUrl } = await this.rpc.resolveMock(
          rawId,
          importer,
          { mock: 'auto' },
        ),
        mockUrl = this.resolveMockPath(cleanVersion(resolvedUrl)),
        mock = this.registry.get(mockUrl)
      if (!mock)
        if (redirectUrl) {
          let resolvedRedirect = new URL(
            this.resolveMockPath(cleanVersion(redirectUrl)),
            location.href,
          ).toString()
          mock = new RedirectedModule(
            rawId,
            resolvedId,
            mockUrl,
            resolvedRedirect,
      ...
      if ('manual' === mock.type) return await mock.resolve()
      if ('automock' === mock.type || 'autospy' === mock.type) {
        let url = new URL(`/@id/${resolvedId}`, location.href),
          query = url.search ? `${url.search}&t=${now()}` : `?t=${now()}`,
          moduleObject = await import(
            `${url.pathname}${query}&mock=${mock.type}${url.hash}`
          )
        return this.mockObject(moduleObject, mock.type)
      }
      return import(mock.redirect)
    }
    ...
    queueMock(rawId, importer, factoryOrOptions) {
      let promise = this.rpc
        .resolveMock(rawId, importer, {
          mock:
            'function' == typeof factoryOrOptions
              ? 'factory'
    ...
    wrapDynamicImport(moduleFactory) {
      return 'function' == typeof moduleFactory
    ...
    resolveMockPath(path) {
      let config = this.config,
        fsRoot = join('/@fs/', config.root)
      return path.startsWith(config.root)
        ? path.slice(config.root.length)
        : path.startsWith(fsRoot)
          ? path.slice(fsRoot.length)
          : path
    }
  },
  versionRegexp = /(\?|&)v=\w{8}/
...
function registerModuleMocker(interceptor) {
  let mocker = new BuildModuleMocker(
    ...
    {
      resolveId: (id, importer) => rpc2('resolveId', { id, importer }),
      resolveMock: (id, importer, options) =>
        rpc2('resolveMock', { id, importer, options }),
      invalidate: async (ids) => rpc2('invalidate', { ids }),
    },
  ...
  return (
    ...
    createCompilerHints({ globalThisKey: '__vitest_mocker__' })
  )
}
globalThis.__STORYBOOK_MOCKER__ = registerModuleMocker(
  () => new ModuleMockerInterceptor(),
)
export { ModuleMockerInterceptor }
