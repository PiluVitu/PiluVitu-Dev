...
- **112 testes** (20 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos) + `pages/home.tsx` + `blocos/Bloco.tsx` + `blocos/BlocoComprometido.tsx` + `blocos/GraficoComprometido.tsx` (incl. `GraficoCategorias`, Task 8) + `lib/commitments.ts` (Task 6, incl. fix round 1, ver seção seguinte) + `blocos/BlocoSaldos.tsx` + `blocos/BlocoDividas.tsx` (Task 7, ver seção _Task 7_ acima) + `blocos/BlocoCategorias.tsx` (Task 8, ver seção _Task 8_ abaixo).
...
- ⚠️ **`src/App.test.tsx#mockFetchVazio` precisou de um branch novo pro mesmo motivo que já tinha um pra `/api/reports/commitments`** — achado ao rodar a suíte inteira (não só os arquivos novos) depois de montar o bloco na home: esse helper devolve `[]` pra QUALQUER rota que não `/api/reports/commitments`, e `BlocoCategorias` também espera um OBJETO de relatório (`{ competence, rows, total_cents }`), não uma lista. Sem o branch, `report.rows.length` tentava ler `.length` de `undefined` (`[].rows`) num `[]` cru, disparando um `TypeError` sem Error Boundary — não falhava a asserção do teste em si (o `<h1>Início</h1>` já tinha montado antes do crash assíncrono), mas aparecia como "2 errors" soltos no relatório do Vitest, silenciosos o bastante pra passar despercebido num `PASS` superficial. Corrigido com o mesmo padrão do branch de commitments: `url.includes('/api/reports/by-category') ? { competence: '', rows: [], total_cents: 0 } : ...`. **Qualquer bloco futuro cujo shape de dado seja um objeto (não lista) precisa do mesmo branch em `mockFetchVazio` — não é automático.**
- **Bundle, medido (`vite build`, antes = Task 7 / depois = Task 8):**

  |                                    | antes                      | depois                     |
  | ---------------------------------- | -------------------------- | -------------------------- |
  | JS principal                       | 273,18 kB / 87,07 kB gzip  | 274,52 kB / 87,43 kB gzip  |
  ...
  Crescimento mínimo nos três — a prova de que reusar o chunk lazy funcionou: o chunk `GraficoComprometido` cresceu só +0,21 kB gzip (o código NOVO de `GraficoCategorias`, não um segundo `recharts`), não os ~104 kB que um chunk duplicado teria custado.
