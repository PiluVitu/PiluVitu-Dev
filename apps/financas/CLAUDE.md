...
- **`transactions.ts`** — `createTransaction` / `createTransfer` / `listTransactions`. O livro-caixa é uma tabela só; os relatórios se separam por filtro, não por tabela.
  ...
  - `createTransfer` aceita `category_id` opcional — sem ele, "pro-labore" era só uma string de descrição (ver seção _Categoria na transferência_ logo abaixo).
...
## Categoria na transferência (`createTransfer` + `POST /api/transfers`)

**O problema medido:** o dono perguntou se já lançava **DAS** e **pro-labore** como despesa da PJ. DAS sim (`kind=expense`). Pro-labore **não** — a categoria semeada tem `kind = "transfer"`, e as duas telas a excluem (`new-entry.tsx:98` filtra por `kind`, `recorrentes.tsx:130` busca `?kind=expense`). ⚠️ **O filtro está CERTO e não deve ser afrouxado:** pro-labore lançado como despesa solta faz o dinheiro sair da PJ **sem entrar** na PF, e contorna a máquina de `transfer_id` exatamente onde ela protege. O que falta é a TELA de transferência (`POST /api/transfers` tem **0 consumidores** em `web/src`); esta fatia prepara o backend dela.

`createTransfer` recebia `{from_account_id, to_account_id, amount_cents, date, description}` e **nenhuma categoria** — as duas pernas nasciam com `category_id` nulo, então nenhum relatório conseguia responder _"quanto tirei de pro-labore este ano?"_. Agora aceita `category_id?: string | null`, e a rota o repassa (contrato de erro intacto: `RangeError` ⇒ `422 invalid_transfer`, constraint ⇒ `constraintResponse`).

⚠️ **A categoria vai SÓ NA PERNA DE SAÍDA, nunca nas duas — decidido pelo modo de falha da alternativa, não por simetria.** Com a categoria nas duas pernas, a consulta mais óbvia que alguém escreveria (`SELECT SUM(amount_cents) … GROUP BY category_id`) devolve **ZERO**: as pernas têm o mesmo valor com sinais opostos e se cancelam. **MEDIDO por mutação** (categoria nas duas pernas): `expected +0 to be -430000` — e `0` é um número plausível ("não tirei nada"), então a resposta errada **não parece errada**, e acertar passaria a exigir que todo consumidor futuro lembrasse de filtrar `amount_cents < 0`. Com a categoria só na saída, a consulta ingênua **e** a com filtro de sinal acertam as duas. A perna de entrada não perde nada irrecuperável: ela carrega o mesmo `transfer_id`, então "o que entrou na PF como pro-labore" é um JOIN pela outra perna.

⚠️ **NÃO se exige `kind = 'transfer'` na categoria, de propósito.** O risco de não exigir é **cosmético e medido**: `cashflow()`, `commitments()` e `byCategory()` filtram `transfer_id IS NULL`, então uma perna marcada como "Alimentação" não infla relatório nenhum — é nonsense inerte. O risco de exigir é **concreto**: a tela de categorias só cria `expense`/`income` (as duas classes estruturais ficam de fora de propósito, ver _A tela de categorias_), então exigir `transfer` prenderia o dono para sempre nas **duas** linhas semeadas (`Pro-labore`, `Transferencia entre contas`) — um "Aporte" novo exigiria SQL manual. Recusar o útil para impedir o inofensivo é a troca errada.

⚠️ **Arquivada RECUSA; inexistente NÃO — a assimetria é a regra.** "Não existe" já tem **dono único**: a FK `transactions.category_id`, que barra igual em `POST /transactions` e em `POST /transfers` (⇒ `422 constraint_violation` com mensagem cozida) — lançar `RangeError` aqui faria o MESMO payload errado responder código diferente conforme a rota. "Arquivada" **não tem dono nenhum**: categoria neste app se **arquiva** (`UPDATE archived_at`), e a FK só olha `DELETE`, então ela nunca dispara; `listCategories` esconde arquivada, logo um id arquivado chegando aqui é sempre estado velho. Aceitar em silêncio é exatamente o defeito já pago em `6ba822c` (a linha nasce numa categoria que nenhuma tela lista, invisível no relatório, sem erro). Diferente da regra que aponta para categoria arquivada — que a tela **mantém**, porque a regra já existia e continua correta —, aqui a linha está **nascendo**: não há estado bom anterior a preservar. A mensagem **não promete desarquivar** (não existe rota de desarquivamento), manda escolher categoria ativa.

**③ Efeito nos relatórios — MEDIDO contra o D1 real, com controles positivos** (`domain/transactions.test.ts`, `'transferencia CATEGORIZADA continua fora de cashflow(), commitments() e byCategory()'`): uma transferência de R$ 4.300 categorizada como Pro-labore, mais uma despesa real de R$ 30 liquidada e uma compra de cartão de R$ 200 prevista. Resultado — `cashflow()`: `saiu_cents = 3000`, `entrou_cents = 0` (a transferência não entra de nenhum lado); `commitments()`: **1** linha, a do cartão, `totals[0] = {min: 20000, max: 20000}`; `byCategory()`: Pro-labore **ausente** da lista, total `-23000`. Os controles positivos são o que impede as três asserções de ausência de passarem por vacuidade contra queries que não veem nada. ⚠️ **`byCategory` não filtra `settled`**, então a compra de cartão aparece no bucket "Sem categoria" — o que **não** aparece é a perna da transferência, que somaria R$ 4.300 e inflaria o mês em ~14× o gasto real.

**④ `accountBalances()` — confirmado, e é o comportamento certo.** Ele soma `amount_cents` sem olhar `transfer_id`: as duas pernas se cancelam, então o **consolidado não muda**, enquanto **cada conta** muda (PJ 900000 → 470000, PF 100000 → 530000 numa transferência de 430000). O dinheiro de fato saiu de uma e entrou na outra; a categoria não altera nada disso.

⚠️ **Verificado por MUTAÇÃO — 3, cada uma matando só o teste certo pelo motivo certo** (todas revertidas; `git status --porcelain -uall` sem resíduo depois):

| Mutação                                        | Falha observada                                                       |
| ---------------------------------------------- | --------------------------------------------------------------------- |
| categoria nas DUAS pernas                      | 2 — `expected +0 to be -430000` (o zero silencioso) + a rota          |
| guard de arquivada removido                    | 2 — `promise resolved instead of rejecting` (grava em silêncio) + 422 |
| `RangeError` também para categoria inexistente | 1 — `expected RangeError … to not be an instance of RangeError`       |
...
⚠️ **E é por ORIGEM, não por COMPRA — ler esta seção sozinha leva à conclusão errada.** `imported_id` só é comparável dentro do esquema de id que o produziu (FITID ≠ hash de CSV ≠ UUID do Pluggy ≠ manual, que não tem nenhum), então a mesma compra vinda por duas portas **não colide e é gravada duas vezes**. Antes de assumir que "reimportar é seguro", ler _O que a dedupe GARANTE, e o que ela NÃO garante_ mais abaixo, com o caso medido no D1 e a heurística que sinaliza o provável.
...
### ⚠️ O que a dedupe GARANTE, e o que ela NÃO garante — a lacuna que passou por três revisões

Esta seção existe porque a ausência dela é o que deixou o defeito passar: cada fatia documentou a **sua** metade da dedupe (`uq_tx_imported` por conta, `:occ:N` dentro do arquivo, `:forcado`, o teto de 500 da conferência) e **nenhuma disse o que o conjunto não cobre**. A tela, por consequência, afirmava mais do que sabia.

| Cenário                                                           | Coberto?                               | Por quem                                    |
| ----------------------------------------------------------------- | -------------------------------------- | ------------------------------------------- |
| Reimportar o MESMO arquivo (mesma origem)                         | ✅ **garantido**                       | `uq_tx_imported` + `importTransactions`     |
| Duas linhas de mesmo id-base **dentro do mesmo arquivo**          | ✅ garantido (viram linhas dis­tintas) | `:occ:N` (`prepararConferencia`)            |
| O mesmo id repetido **dentro da mesma requisição**                | ✅ garantido                           | `seenInThisRequest` (`domain/import.ts`)    |
| Forçar a mesma duplicata duas vezes                               | ✅ garantido (id determinístico)       | sufixo literal `:forcado`                   |
| **A mesma compra por DUAS ORIGENS** (OFX × Pluggy × CSV × manual) | ❌ **IMPOSSÍVEL de garantir**          | só o palpite de `lib/duplicata-provavel.ts` |
| Colisão contra lançamento fora dos 500 mais recentes da conta     | ❌ não visto na conferência            | (o servidor ainda barra, se o id bater)     |

⚠️ **A causa da linha ❌ é ESTRUTURAL, não um bug a corrigir:** `uq_tx_imported` é `(account_id, imported_id)`, e cada origem carimba um esquema de id diferente **por natureza** — OFX usa o FITID do banco, o CSV usa o hash `idEstavel` (`@piluvitu/tools/import/id`), o Pluggy usa um UUID próprio, e lançamento manual **não tem `imported_id` nenhum**. A mesma compra vinda por duas portas nunca colide: não existe colisão pro índice ver. **MEDIDO no D1 real** — a mesma compra (`2026-07-10`, `-18990`, `SUPERMERCADO XPTO`) importada por OFX e depois por Pluggy deu `skipped: 0`, **2 linhas**, `SUM(amount_cents) = -37980`, o dobro; e a tela dizia literalmente _"1 linha(s) encontrada(s). 0 já parecem importadas — desmarcadas por padrão"_, com o checkbox **PRÉ-MARCADO**.

⚠️ **O estrago real, e por que ele não dá erro nenhum:** o dono importa 12 meses por OFX hoje, liga o Pluggy amanhã e sincroniza a mesma janela — dobra o histórico inteiro. `accountBalances`, `cashflow` e `byCategory` erram por 2×, e nada na tela indica isso. Pior, é irreversível pelo caminho normal: `uq_tx_imported` impede reimportar por cima pra corrigir, então desfazer é `DELETE ... WHERE import_source='pluggy'` ou Time Travel (que restaura o banco INTEIRO). A fatia do Pluggy **não criou** o buraco (ele existe desde que há mais de uma origem), mas o tornou provável.

**O que foi feito, já que dedupe exata entre origens é insolúvel: (a) a tela parou de afirmar o que não sabe, e (b) o provável passou a ser sinalizado.**

- **(a) A frase.** O resumo da conferência agora separa as duas checagens, diz sobre o que cada uma vale e chama o palpite de palpite (`conferencia-resumo` + `conferencia-limite-dedupe`). O texto de `<Ajuda rotulo="Duplicata">` foi reescrito no mesmo espírito.
- **(b) A heurística — `web/src/lib/duplicata-provavel.ts`, pura e testada isolada.** Uma linha é PROVÁVEL duplicata quando existe, na mesma conta, lançamento gravado de **ORIGEM DIFERENTE** com **`amount_cents` exato ao centavo** e **`purchase_date` dentro de ±1 dia**.

⚠️ **As três decisões do critério, cada uma pesada contra o caso real que ela cria:**

1. **Só cruza ORIGENS DIFERENTES.** Dentro da mesma origem os ids SÃO comparáveis e a dedupe exata já é autoritativa — a heurística ali só somaria falso positivo. É essa restrição que desarma o contra-exemplo óbvio: **dois cafés de R$ 12 no mesmo dia, no MESMO extrato, não se marcam.** E é o mesmo raciocínio que faz a checagem valer exatamente onde o índice não alcança — inclusive contra **lançamento manual**, que nunca teve `imported_id` e por isso é o caso em que a dedupe exata é 100% cega (lançar o aluguel à mão e depois importar o extrato).
2. **Cada lançamento gravado é consumido por, no máximo, UMA linha nova.** Já existe UM café de R$ 12 no dia e o Pluggy traz DOIS ⇒ só o primeiro é marcado; o segundo é genuinamente novo e nasce marcado. Sem o consumo, um único lançamento antigo contaminaria todas as linhas de mesmo valor no dia.
3. ⚠️ **A DESCRIÇÃO NÃO É CONDIÇÃO — só reforço.** É a decisão mais contraintuitiva e a mais importante. Origens diferentes descrevem a MESMA compra com strings diferentes **por construção**: OFX carrega o `MEMO` do banco (`COMPRA CARTAO 10/07 SUPERMERCADO XPTO`), o Pluggy devolve `description` já limpa (`Supermercado Xpto`), o CSV traz a coluna que o dono mapeou e o manual traz o que ele digitou ("mercado"). **Exigir que batam faria a checagem falhar justamente no cenário que ela existe pra pegar** — medido por mutação: transformar a descrição em condição derruba 11 testes. `normalizeName` (o mesmo de `payee-suggest.ts`, nunca uma segunda normalização) entra só pra dizer no aviso se as descrições coincidem, e pra desempatar candidatos na mesma data.

⚠️ **A janela de ±1 dia não é folga arbitrária:** é a conversão de fuso que a fatia do Pluggy acabou de tratar. O app é `purchase_date`-cêntrico em GMT−3 e o Pluggy entrega `date` em UTC (compra às 22h local é 01h UTC do dia seguinte); o OFX carimba a data que o BANCO decidiu, que pode ser a de processamento. As duas origens legitimamente discordam em um dia sobre a mesma compra, e exigir data exata deixaria passar justo a compra noturna.

⚠️ **ERRA PRO LADO DO FALSO POSITIVO — escolha explícita, porque os dois erros não custam o mesmo.** Falso positivo (marcar legítimo como provável duplicata) custa ao dono **um clique** pra remarcar, com a linha colidente à vista — julgamento imediato. Falso negativo custa **dinheiro contado duas vezes**, irreversível pelo caminho normal (o parágrafo do estrago, acima). Entre marcar demais e deixar passar, **marcar demais**. As decisões 1 e 2 existem exatamente pra manter o falso positivo raro em vez de aceitá-lo de braços cruzados: **marcar como duplicata o que é obviamente legítimo ensina o dono a ignorar o aviso — e aí ele ignora o verdadeiro, que é o único que custa dinheiro.**

**Na tela:** a linha provável nasce **desmarcada**, com badge `Talvez duplicada` (variante `outline`, deliberadamente mais fraca que o `destructive` da certeza) e uma frase que nomeia **o motivo e a linha com que colide** — data, valor, descrição e a origem por extenso (`rotuloOrigem`), mais se a descrição bate ou não. ⚠️ **O dono pode marcar mesmo assim** — a heurística erra, e a tela não pode ser mais confiante que o método. ⚠️ **Marcar de volta envia o `imported_id` NATURAL, nunca `:forcado`:** aquele sufixo existe pra contornar uma colisão real de id, e aqui o id não colide com nada — é justamente por isso que a heurística precisou existir. ⚠️ **A certeza vence o palpite:** quando o `imported_id` já existe, a linha mostra só o motivo exato.

⚠️ **CUSTO NO D1 DA CHECAGEM NOVA: ZERO linha lida a mais — MEDIDO, não estimado.** A conferência já buscava `GET /api/transactions?account_id=…&limit=500` pra dedupe exata, e `listTransactions` faz `SELECT` das **20 colunas**: `purchase_date`, `amount_cents`, `description` e `import_source` **já vinham nesse payload e eram descartados no cliente**. Nenhuma query nova foi acrescentada. Medido contra o D1 (Miniflare) com 300 lançamentos na conta: `meta.rows_read = 600` (2 por linha — índice `idx_tx_account_date` + tabela), plano `SEARCH transactions USING INDEX idx_tx_account_date (account_id=?)`, idêntico antes e depois. A heurística herda também o teto de 500 já documentado (linha ❌ da tabela): numa conta com mais de 500 lançamentos mais novos, uma colisão antiga não é vista — mesma limitação, e mesma honestidade, da dedupe exata.

**Suítes:** SPA **581 → 604** (+23: 16 em `lib/duplicata-provavel.test.ts`, 7 em `pages/importar.test.tsx`). Worker **810**, `packages/tools`, `apps/web` e `packages/ui` **intocados** — nenhuma linha de servidor, nenhuma rota, nenhuma migration. `tsc --noEmit` e prettier limpos; os dois gates de build (`check-tailwind-source.mjs`, `check-financas-lazy-chart.mjs`) com exit 0 e **um** único chunk lazy. ⚠️ **Nenhuma asserção pré-existente mudou de valor.**

⚠️ **Verificado por MUTAÇÃO — 10, cada uma matando só o teste certo pelo motivo certo** (todas revertidas por **cópia de arquivo**, nunca `git checkout`; `git status --porcelain -uall` mostrando só os 4 arquivos da fatia depois):

| Mutação                                                        | Falha observada                                                 |
| -------------------------------------------------------------- | --------------------------------------------------------------- |
| heurística desligada (sempre `null`)                           | 13                                                              |
| filtro de origem removido (cruza qualquer origem)              | 2 — os dois cafés do mesmo OFX voltam a se marcar               |
| consumo removido (um gravado contamina várias linhas)          | 1                                                               |
| **descrição vira CONDIÇÃO** (o critério inseguro)              | 11 — a checagem falha justo no cenário que ela existe pra pegar |
| `JANELA_DIAS = 0` (exige data exata)                           | 3 — a compra noturna deixa de ser vista                         |
| provável NÃO desmarca (`marcada: !duplicada`, o estado do bug) | 2                                                               |
| a frase volta ao texto antigo                                  | 2                                                               |
| `:forcado` aplicado também à provável                          | 1                                                               |
| o bloco do motivo some da linha                                | 3                                                               |
| o palpite se sobrepõe à certeza                                | 1                                                               |

**Bundle (`vite build`, antes = fatia do Pluggy / depois = esta):** JS principal 510,92 → 513,89 kB (**153,85 → 154,89 kB gzip, +1,04**); CSS **6,99 kB gzip, intocado**; chunk lazy `GraficoComprometido` **113,31 kB gzip, intocado**. Nenhuma dependência nova.

**Fora de escopo, registrado:** a heurística roda só na CONFERÊNCIA do import — o extrato não sinaliza duas linhas suspeitas já gravadas, e não há rota que responda "estas N linhas parecem duplicadas". Se um dia fizer falta, o critério já é puro e reusável (`lib/duplicata-provavel.ts`); o que faltaria é onde mostrar e o que oferecer (apagar uma das duas exige `DELETE /api/transactions/:id`, que já existe).
...
### Fix pós-fatia — três defeitos MEDIDOS depois de `bf9f5a6`

Nenhuma migration, nenhum código de erro novo, nenhuma mudança de shape de rota. Os três eram de LEITURA/robustez, e os três tinham o mesmo formato: a tela dizia (ou fazia) uma coisa diferente do que o dado dizia.

**① `/api/rules` saiu do `Promise.all` FATAL de `pages/importar.tsx`.** MEDIDO em Chrome real com a rota devolvendo `500 internal_error`: `#/importar` renderizava **só** um `<p role="alert">erro interno</p>` — sem `<h1>`, sem select de conta, sem input de arquivo. ⚠️ **E é alcançável AGORA:** a migration `0009` (que cria a tabela `rules`) só entra em produção por ação manual do dono, então publicar o Worker antes dela derruba a tela de import, que hoje funciona (provado contra o Worker real: `DROP TABLE rules` → `GET /api/rules` → 500). É a **classe C2** já registrada neste arquivo (a tabela `settings` ausente 500ava TRÊS telas), e o próprio commit da fatia já tinha o padrão certo em `regras.tsx#carregarMatches` — copiado de lá, não inventado um terceiro jeito. A regra: **uma capacidade OPCIONAL que falha não pode derrubar a capacidade PRINCIPAL.** Importar não precisa de regra nenhuma — sem elas a sugestão volta a sair só do `payees.default_category_id`, que é como o import funcionava antes de as regras existirem. **Degradado não é silencioso:** `regrasIndisponiveis` renderiza um `role="alert"` abaixo do `<h1>` dizendo que as sugestões vieram só do favorecido — sem isso o dono conferiria dezenas de linhas sem categoria e concluiria que as regras dele pararam de funcionar.

**② O formulário de edição de `pages/regras.tsx` exibia o OPOSTO do que a regra faz.** MEDIDO renderizando `RegrasPage` e clicando "Editar": categoria arquivada ⇒ o select exibia **"Não mexe"** enquanto o estado guardava `c-arquivada`, e salvar sem tocar mandava `"set_category_id":"c-arquivada"`; conta arquivada ⇒ exibia **"Qualquer conta"** e o PUT mandava `"match_account_id":"a-arquivada"`. Causa: conta e categoria se **arquivam** (não se apagam), `GET /api/accounts|categories` escondem arquivada, e arquivar não é `DELETE` — o `ON DELETE CASCADE` da `0009` nunca dispara, então a regra segue apontando pra um id que sumiu da lista que alimenta o `<select>`; sem `<option>` casando o valor, o select caía na primeira.

⚠️ **O caso da CONTA é o mais grave, e é irônico:** "a regra alarga sozinha, em silêncio" é exatamente o que o CASCADE da `0009` foi escrito pra impedir. O schema impede de verdade — era a TELA que exibia como se tivesse acontecido. O dono lê "Qualquer conta", acredita, e salva.

**A decisão: MOSTRAR a opção arquivada, marcada e nomeada (`Conta arquivada ou removida (mantida)`), com um `role="alert"` ao lado — e NÃO bloquear o salvamento.** Justificativa: (a) uma regra "só no cartão X" continua **CERTA** depois de o cartão ser arquivado — os lançamentos daquela conta continuam existindo e a regra continua casando exatamente eles; o inaceitável nunca foi a regra apontar pra conta arquivada, foi a tela MENTIR sobre isso; (b) bloquear recusaria uma edição que não tem nada a ver com o campo arquivado (renomear a regra, mudar a Ordem) — mesma classe do `PATCH` que manda só o que mudou em `extrato.tsx`, onde recusar o campo intocado impediria uma correção legítima; (c) o dono pode querer **manter** a regra como está, e o aviso nomeia a saída (`"Qualquer conta"` ALARGA; `"Não mexe"` desliga a ação) em vez de impô-la. A LISTA já era honesta (`na conta (removida)`, `(arquivada ou removida)`); o formulário passou a ser também. **Favorecido não precisou do mesmo tratamento:** `set_payee_id` é CASCADE e payee não se arquiva — id pendurado é impossível por essa porta.

**③ `/matches` continua contando ativas E pausadas — o que mudou foi o TEMPO VERBAL.** MEDIDO: uma regra pausada idêntica a uma ativa recebia `counts = 5`, e a tela renderizava **"Casaria 5 de 6 lançamento(s) já existentes."** ao lado do badge **"Pausada"** — afirmando um efeito presente que a regra não tem. **A decisão: NÃO filtrar `active = 1` na rota.** Filtrar seria mais simples e entregaria menos: o número é justamente o que responde _"vale a pena reativar esta?"_, e uma regra pausada é exatamente onde essa pergunta é feita — filtrando, a tela ficaria muda sobre a única regra cujo futuro o dono está decidindo, e a alternativa dele seria reativar às cegas pra descobrir. Quem paga o preço é a frase: `active = 0` agora lê **"Pausada, não casa nada hoje — se reativada, casaria N de M lançamento(s) já existentes."**; `active = 1` segue byte a byte na afirmação direta de antes. O contrato ficou explícito no comentário de `/matches` (`src/routes/rules.ts`) **e travado por teste** (`rules.test.ts`, "regra PAUSADA também é contada"), pra ninguém "consertar" a rota depois filtrando.

**Suítes:** Worker **678 → 679** (+1 em `routes/rules.test.ts`); SPA **500 → 507** (+5 em `pages/regras.test.tsx` — 2 de ②, 1 controle positivo de ②, 2 de ③ incl. o controle da regra ativa —, +2 em `pages/importar.test.tsx`); `packages/tools` **147, intocado** (o motor não mudou); `apps/web`/`packages/ui` intocados. `tsc --noEmit` limpo nos dois lados, `prettier --check` limpo, os dois gates de build (`check-tailwind-source.mjs`, `check-financas-lazy-chart.mjs`) com exit 0 e **um** único chunk lazy. ⚠️ **Nenhuma asserção pré-existente mudou de valor** — a frase da regra ATIVA foi mantida idêntica de propósito, pra o teste de contagem que já existia continuar sendo prova.

⚠️ **Verificado por MUTAÇÃO — 4, cada uma matando só o teste certo pelo motivo certo** (todas revertidas; `git status --porcelain -uall` mostrando só os 6 arquivos da correção depois):

| Mutação                                                           | Falha observada                                                                    |
| ----------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| ① `/api/rules` de volta no `Promise.all` fatal (o estado pré-fix) | 1 — a tela inteira vira o alerta, `heading "Importar"` some                        |
| ② `contaSumida`/`categoriaSumida` fixos em `false`                | 2 — os selects voltam a exibir "Qualquer conta"/"Não mexe" com o id arquivado      |
| ③ frase no presente do indicativo pra TODA regra                  | 1 — a pausada volta a afirmar "Casaria 5 de 6" ao lado do badge                    |
| ③ (Worker) `listRules(db, { onlyActive: true })` em `/matches`    | 1 — `expected undefined to be 1`: a pausada some da contagem que decide reativá-la |

**Bundle (`vite build`, antes = a fatia de regras / depois = este fix):** JS principal 490,26 → 491,77 kB (**147,82 → 148,22 kB gzip, +0,40** — dois avisos de texto e duas `<option>` condicionais, nenhuma dependência nova); chunk lazy `GraficoComprometido` **113,31 kB gzip, intocado**; CSS **6,79 kB gzip, intocado**.
...
### As rotas que faltavam — criar/editar/arquivar categoria e editar payee (`src/domain/categories.ts`, novo)

⚠️ **O PROBLEMA, medido na produção real: o dono tinha 7 categorias e não conseguia criar a oitava.** Eram as semeadas pela migration (Contador, Custos da PJ, DAS, INSS, Pro-labore, Quitação de dívida, Transferência entre contas) — ou seja, **"Mercado", "Gasolina" e "Almoço" não tinham onde ser registrados**. `src/routes/categories.ts` tinha 45 linhas e **só** `GET /` (linha 25). **ZERO migration**: `parent_id`, `slug`, `default_scope` e `archived_at` existem no schema desde o `0001` (`0001:81-106`); faltava código que os escrevesse.

| Rota                               | Sucesso | Erros                                                                               |
| ---------------------------------- | ------- | ----------------------------------------------------------------------------------- |
| `POST /api/categories`             | 201     | 400 `invalid_json`, 422 `constraint_violation`                                      |
| `PUT /api/categories/:id`          | 200     | 400 `invalid_json`, 404 `not_found`, 422 `constraint_violation` / `protected_field` |
| `POST /api/categories/:id/archive` | 200     | 404 `not_found`, 422 `constraint_violation`                                         |
| `PUT /api/payees/:id`              | 200     | 400 `invalid_json`, 404 `not_found`, 422 `constraint_violation` / `protected_field` |

**Nenhum código de erro novo no catálogo** — `protected_field` já existia (`PATCH /api/transactions/:id`), e é reusado aqui com o mesmo contrato (`field` nomeia a chave ofensora).

As cinco decisões, cada uma com o motivo:

- ⚠️ **`slug` NUNCA é aceito do cliente.** Ele é IDENTIDADE semeada pela migration (`0001:92-94`): `payDebt()` acha a categoria de quitação por `WHERE slug='quitacao-divida'`, e o gap da PJ é medido por `das`/`contador`/`inss`/`pro-labore`. Categoria criada pelo dono nasce com `slug NULL`, e por isso `uq_categories_slug` (índice PARCIAL, `WHERE slug IS NOT NULL`) é impossível de violar por esta porta. **MEDIDO por mutação:** passando `body.slug` adiante, o `POST` com `{slug:'das'}` deixa de ser 201 e vira **422**, colidindo com a linha semeada — ou seja, a porta aberta permitiria sequestrar a identidade que o CÓDIGO procura.
- ⚠️ **`kind` é IMUTÁVEL no `PUT` — e a recusa é explícita (422 `protected_field`), não um "ignora e segue".** Trocar o `kind` de uma categoria que já tem lançamentos reclassificaria o histórico em silêncio (despesa virando receita, ou virando `transfer`, que todo relatório de resultado exclui). A checagem roda **antes de qualquer escrita e vale pro corpo inteiro**: `{name, kind}` não renomeia e ignora o resto — teste confere a coluna no D1 depois do 422. Mesma regra e mesmo código de `PATCH /api/transactions/:id`. `slug`, `archived_at`, `id` e `created_at` entram na mesma lista.
  - **Assimetria deliberada POST × PUT:** no `POST`, `slug` (e qualquer chave desconhecida) é **ignorado**, porque o 201 devolve a linha inteira e o cliente lê `"slug": null` no ato. No `PUT` ele manda só o delta e já tem o objeto — um campo dropado em silêncio vira uma alteração que ele acredita ter feito.
- ⚠️ **Arquivar, NUNCA `DELETE`.** `transactions.category_id` é `ON DELETE SET NULL` (`0001:170`): um `DELETE FROM categories` teria **sucesso** e descategorizaria todo o histórico daquela categoria, sem erro nenhum. `UPDATE ... WHERE id=? AND archived_at IS NULL` → `boolean` de `meta.changes` → 404 (convenção de `archiveAccount`: id inexistente e já-arquivada casam zero vezes). **MEDIDO por mutação:** trocando o `UPDATE` por `DELETE`, 5 testes caem — incluindo os dois que leem a linha de volta do D1 (`expected undefined to be '<id>'`).
- ⚠️ **Arquivar mãe com filhas ATIVAS: RECUSA (422).** `listCategories` esconde arquivada, então arquivar só a mãe deixaria a filha visível pendurada num pai que sumiu da tela. É o caso REAL do seed: `Custos da PJ` é mãe de DAS/Contador/INSS (`0001:486-497`). Filha já arquivada não conta (não há órfã visível a criar) — teste prova a sequência filha→mãe passando.
- ⚠️ **Hierarquia validada em TS, porque o `CHECK` do schema não basta.** `0001:100` é só `CHECK (parent_id IS NULL OR parent_id <> id)` — ele barra **um** caso e nada além: profundidade 3 passa, e o ciclo A→B→A passa (nenhuma das duas linhas aponta pra si mesma). A regra usada é a de `createAccount` (validar antes, com mensagem legível): **o pai precisa existir e ser RAIZ (`parent_id IS NULL`)**, o que trava a árvore em 2 níveis e mata o ciclo de brinde (num ciclo, B teria que ser pai de A, mas B já é filha). No `PUT` há um segundo guard independente: quem **já é mãe** não pode virar filha (senão a neta nasce no mesmo instante) — este conta TODAS as filhas, inclusive arquivadas, porque profundidade é invariante de ESTRUTURA, não de visibilidade (o guard de arquivamento, ao contrário, só olha as ativas). **MEDIDO por mutação:** matando o guard de raiz, 7 testes caem; matando só o de "já é mãe", o ciclo A→B→A continua sendo recusado — pela outra guarda, com outra mensagem —, e cai o teste da filha arquivada.
- **Mãe arquivada não recebe filha nova** (mesmo estado do bullet acima, criado pela porta de trás).

**`GET /api/categories` ganhou `LIMIT 500`** (mesma família de `BY_CATEGORY_LIMIT`/`listTransactions`): até aqui a lista era um punhado curado à mão pela migration e o teto seria decorativo — a partir de `createCategory` ela cresce pela mão do dono, que é exatamente quando um teto deixa de ser decorativo. `archived_at` passou a sair no payload (aditivo); `listCategories` aceita `includeArchived` no domínio, e a rota continua escondendo arquivada (sem parâmetro novo).

**② `PUT /api/payees/:id` — o buraco real.** `POST /api/payees` **sempre aceitou** `default_category_id` (`routes/payees.ts:50-53`, `domain/payees.ts:113-121`); quem nunca o ENVIA é o único chamador da SPA, `DividasPage.tsx:128-131`, que posta só `{name, kind:'person'}`. Sem `PUT`, o campo de um payee já criado era **imutável** — e `payee-suggest.ts:83` → `importar.tsx:251` lê exatamente esse campo, então a cadeia era: campo existe → ninguém preenche → **a sugestão do import sempre vinha sem categoria**. Agora dá pra ENSINAR um payee existente.

- ⚠️ **`norm_name` é RECALCULADO junto com `name`, na mesma statement — e é a parte que importa.** Ele é a CHAVE DE MATCHING do import (`idx_payees_norm`, casado contra a descrição normalizada da linha importada), não um campo de exibição: renomear e deixar o `norm_name` velho faria o payee continuar casando com o estabelecimento ANTIGO, em silêncio, sugerindo a categoria errada em toda importação seguinte. Por isso `norm_name` também é **campo protegido** no corpo (derivado; um segundo caminho de escrita poderia contradizer o `name`). **MEDIDO por mutação:** tirando o recálculo, 2 testes caem com `expected 'MERCADO SAO LUIZ' to be 'PADARIA DO ZE'`.
- **Categoria inexistente em `default_category_id` sai como `422 constraint_violation` cozido** — `payees.default_category_id` tem FK real (`REFERENCES categories(id) ON DELETE SET NULL`), então a recusa vem do D1 e passa por `friendlyConstraintMessage`/`logConstraintError` (o texto cru, com nome de tabela/coluna, só vai pro `console.error`). Confirmado por teste com asserção NEGATIVA (`not.toMatch(/SQLITE|D1_ERROR|FOREIGN KEY|payees/i)`) — a FK é enforçada de fato pelo D1, não assumida.
- `kind`/`id`/`created_at` também são protegidos: esta rota corrige nome, documento e categoria padrão.

Testes: `pnpm --filter @piluvitu/financas exec vitest run src/domain/categories.test.ts src/routes/categories.test.ts src/domain/payees.test.ts src/routes/payees.test.ts` (21 + 19 + 14 + 13 = 67). **Worker: 577 → 627** (33 → 34 arquivos); **SPA intocada (364)** — esta fatia é só servidor, nenhuma tela consome as rotas novas ainda. `tsc --noEmit` e prettier limpos. ⚠️ **Verificado por MUTAÇÃO, 7 vezes, cada uma matando o teste certo pelo motivo certo** (todas revertidas; `git status --porcelain -uall` sem resíduo depois): guard de arquivamento (3 testes), guard de raiz (7), guard de "já é mãe" (3), `slug` do cliente (1), `norm_name` sem recálculo (2), campo protegido virando "ignora e segue" (3), `DELETE` no lugar de arquivar (5).

✅ **As TELAS chegaram na fatia seguinte** — ver _A tela de categorias_ + _Categoria e favorecido na tela Lançar_ logo abaixo. Continua **não existindo rota de DESARQUIVAR** categoria (o `archived_at` só é escrito, nunca limpo) — a saída hoje é `wrangler d1 execute` manual, e a tela diz isso no diálogo em vez de sugerir uma volta que não existe. `PUT /api/payees/:id` também **segue sem tela**: nenhuma edita um payee já criado; o que passou a existir é um ESCRITOR de `default_category_id` na criação (o "— novo favorecido —" da tela Lançar).

## A tela de categorias (`web/src/pages/categorias.tsx`, rota `#/categorias`)

O consumidor que faltava para as rotas acima: até aqui o dono tinha as 7 categorias semeadas e **nenhuma forma de criar a oitava** pela interface. Molde de `pages/recorrentes.tsx` — lista + UM formulário reusado pra criar/editar via `editandoId` + `Dialog` de `@piluvitu/ui/dialog` pra confirmar o arquivamento (⚠️ **nunca `window.confirm()`** — o modo de falha medido no Chrome Android, ver `debt-detail.tsx`).

- ⚠️ **A hierarquia é RENDERIZADA pela primeira vez.** `Custos da PJ` é mãe de DAS/Contador/INSS desde a migration (`0001:486-497`), `parent_id` sempre saiu no `GET`, e **nenhuma tela jamais mostrou isso**. `ordenarPorHierarquia` (`web/src/lib/categories.ts`, junto com o tipo `CategoryView`) achata a árvore em `{categoria, nivel}` — cada raiz seguida das próprias filhas, alfabético dentro de cada grupo; a filha renderiza com `ml-4 border-l pl-3` e `data-nivel="1"`. Mora em `lib/` porque nasceu com DOIS consumidores (esta tela e o `<select>` de Lançar), mesma disciplina de `CommitmentReportView`.
  - ⚠️ **Filha cuja mãe não está na lista recebida (mãe arquivada, ou lista filtrada por `kind` no cliente) vira RAIZ, no fim — nunca some.** Descartá-la esconderia uma categoria real por causa de um filtro sobre OUTRA linha; deixá-la em `nivel: 1` renderizaria indentação pendurada em nada. É o único ramo que nenhum teste de tela alcança sozinho, e por isso tem caso próprio em `lib/categories.test.ts`.
- ⚠️ **Só `expense` e `income` são criáveis.** `transfer`/`debt_settlement` são estruturais: as duas classes que todo relatório de resultado exclui, e as linhas semeadas são achadas pelo CÓDIGO por slug (`payDebt()` procura `quitacao-divida`). Uma categoria nova desse tipo nasceria sem slug e só serviria pra classificar gasto real numa classe que os relatórios ignoram. As seedadas continuam na lista (existem, e nome/mãe delas é editável) — o que não existe é a porta pra criar mais.
- ⚠️ **`kind` NUNCA vai no corpo do `PUT`** — a rota o recusa com `422 protected_field`, e **a recusa vale pro corpo INTEIRO**: mandá-lo "só por mandar" faria a renomeação inteira falhar. Em edição o campo vira texto (`tipo-fixo`), não um `<select>`.
- **Opção que o servidor sempre recusaria não é oferecida** (mesma disciplina do `<select>` de conta em `debt-detail.tsx`, que esconde `credit_card` porque `payDebt` sempre recusa): o select de mãe lista só raízes e nunca a própria categoria em edição; quem **já é mãe** perde o campo, substituído por um aviso de que a hierarquia tem 2 níveis.
- **Arquivar, com o texto conferido contra `archiveCategory`**: a categoria some das listas/seletores, **nada é apagado** (`transactions.category_id` continua apontando pra ela) e **não há como desarquivar pela interface** — o diálogo diz isso em vez de prometer uma volta inexistente. Mãe com filha ATIVA é `422`, e a mensagem vem **crua do domínio** ("arquive as filhas primeiro"), que é quem sabe nomear a saída.
- **`mutarERecarregar` nas três mutações** — a mensagem de recarga da CRIAÇÃO nomeia o estrago do reenvio ("criaria uma segunda categoria com o mesmo nome, e o relatório por categoria mostraria o mesmo gasto partido em duas linhas").

## Categoria e favorecido na tela Lançar (`web/src/pages/new-entry.tsx`)

Os dois campos que faltavam pro dono dizer **para onde foi o dinheiro** — `grep -ci categor|payee` em `new-entry.tsx` dava **0** antes desta fatia. **Zero mudança de servidor**: `POST /api/transactions` já repassava o corpo direto, e `routes/installments.ts` já lia os dois campos e os propagava pra CADA parcela. Dois `<select>` entre "Conta" e "Recorrente"; `Promise.all` de 2 → 4 (`/accounts`, `/recurring`, `/categories`, `/payees`).

- ⚠️ **O filtro de `kind` segue o checkbox "Entrada"** (`income` marcado, `expense` desmarcado) e é feito **no cliente**, sem `?kind=` na URL — refetch a cada toggle gastaria uma requisição por clique pra reordenar meia dúzia de linhas que já estão na mão. Como esses são os dois únicos valores que a comparação pode casar, **`transfer` e `debt_settlement` ficam SEMPRE de fora** — deixar o dono escolher "Quitação de dívida" à mão recriaria a dupla contagem que o slug dedicado de `payDebt()` existe pra impedir.
- ⚠️ **Trocar o "Entrada" LIMPA a categoria que não vale mais.** Sem isso o `<select>` renderiza vazio (o valor não casa com nenhuma `<option>`) mas o ESTADO segue com o id velho, e o lançamento sairia com uma categoria do tipo errado, em silêncio. Um `useEffect` sobre a lista visível zera a seleção; teste dedicado submete depois do toggle e exige a chave AUSENTE do corpo.
- ⚠️ **Envio: `category_id: categoryId || undefined`** — `JSON.stringify` OMITE a chave, nunca uma string vazia (mesma disciplina de `recurring_expense_id`: quem decide no backend é a AUSÊNCIA). Coberto por asserção `'category_id' in body === false`, não só por `toEqual`.
- ⚠️ **Os dois selects NÃO somem quando "Parcelado" está marcado** — o backend do ramo parcelado aceita os dois e os propaga pra cada parcela. Diferente do select "Recorrente", que é escondido de propósito.
- **Indentação da hierarquia dentro do `<option>` usa ` ` (espaço não-quebrável), nunca espaço comum** — o navegador colapsa/apara espaço comum no rótulo de uma `<option>`, e a indentação (a única forma de mostrar hierarquia num `<select>` nativo) sumiria. MEDIDO no Chrome real: `'  ↳ DAS — Simples Nacional'` chega inteiro.
- ⚠️ **"— novo favorecido —" é o PRIMEIRO ESCRITOR REAL de `payees.default_category_id`.** `POST /api/payees` sempre aceitou o campo e ninguém nunca o enviava (`DividasPage` posta só `{name, kind:'person'}`) — por isso `payee-suggest.ts` → `importar.tsx` **sempre sugeria sem categoria**. O favorecido novo é criado com um POST próprio ANTES do POST principal, dentro do callback de mutação (padrão de `DividasPage`), levando junto a categoria escolhida na tela; o id devolvido é o que vai no lançamento, nunca o sentinel.
- **`mutarERecarregar`**: a recarga existe pra o favorecido recém-criado aparecer no `<select>` da próxima vez, e um GET que cai depois do 201 nunca pode ler como "o lançamento falhou" (reenviar duplicaria dinheiro).

**Mobile (390 px), MEDIDO em Chrome real** (`vite build` + `vite preview`, viewport 390×844 com `hasTouch`/`isMobile`): `scrollWidth === clientWidth === 390` nas duas telas. `#/categorias` é lista de cards, **nunca tabela** — nenhuma coluna a cortar (a lição de `DividasPage`/`debt-detail`), e a hierarquia aparece como indentação real (raiz em `x=41`, filha em `x=57`, nenhuma linha com `scrollWidth > clientWidth`); o `Dialog` de arquivar cabe exato (`x:0, width:390`). Em `#/lancar` os dois selects ocupam a largura toda do card (308 px cada) e a indentação com ` ` sobrevive no rótulo.

**Suítes:** SPA **364 → 389** (32 → 34 arquivos: +12 `pages/categorias.test.tsx`, +8 `pages/new-entry.test.tsx`, +3 `lib/categories.test.ts`, +2 `App.test.tsx`). Worker **627, intocado** (fatia só de tela). `tsc --noEmit` e prettier limpos; os dois gates de build (`check-tailwind-source.mjs`, `check-financas-lazy-chart.mjs`) silenciosos. Bundle: JS principal 429,85 → 441,04 kB (132,35 → 135,08 kB gzip, +2,73 kB — tela nova + dois selects, nenhuma dependência nova); chunk lazy do gráfico intocado (388,47 kB / 113,09 kB gzip).

⚠️ **Verificado por MUTAÇÃO, 7 vezes, cada uma matando só o teste certo pelo motivo certo** (todas revertidas, `git status --porcelain -uall` sem resíduo): hierarquia achatada em ordem alfabética global (4 testes, incl. os rótulos do `<select>`); `category_id`/`payee_id` como string vazia em vez de omitidos (4 — os dois `toEqual` pré-existentes de corpo caem junto); efeito de limpeza da categoria removido (1); `kind` no corpo do `PUT` (1); arquivar pulando o `Dialog` (3, todos morrendo na ABERTURA do diálogo — `Unable to find role="heading" and name "Arquivar categoria"`); filtro de `kind` removido do `<select>` (2 — transferência e quitação reaparecem); `carregar()` de volta pra dentro do callback de mutação em `categorias.tsx` (1).
...
As duas contas nasceram com **saldo negativo** (Nubank RDB −105,39, Inter CDB −50,37) porque `opening_balance_cents` começou em 0 e a **primeira** movimentação de cada uma é um RESGATE: resgatar de um cofre que, nos livros, começou vazio deixa o cofre devendo. O dinheiro aplicado antes de out/2025 está fora da janela que o Pluggy trouxe.

`migrations/0014_saldo_abertura_investimento.sql` fecha isso, e **os valores não são estimativa**: o dono confirmou que esvaziou as duas e não mexeu mais (coerente com o dado — última movimentação 2025-10-17, nada depois). Se o saldo hoje é zero e o líquido conhecido é −105,39, o saldo de abertura era exatamente +105,39; mesma conta para o Inter CDB (+50,37). Verificado na simulação: `abertura 105,39 + movimento −105,39 = saldo 0,00`, e idem no Inter.

⚠️ **Saldo negativo numa conta aqui quase sempre é `opening_balance_cents` faltando, não erro de lançamento.** Vale para as contas correntes também: `Inter Empresa` (−63,00) e `Nubank` (−8,75) seguem assim pelo mesmo motivo, e o conserto é o mesmo campo — editável em `#/contas`, sem migration. Só virou migration nas duas de investimento porque ali os valores foram **derivados** do ledger, e derivação merece ficar registrada junto com o raciocínio.
...
⚠️ **A fatia de contraste dos tokens (posterior) mexeu nesses dois números e a conclusão NÃO mudou — piorou de leve: claro 1,80 → 1,48, escuro 1,31 (intocado).** Escurecer `--primary` e `--destructive` para os dois passarem em AA sobre o fundo claro aproxima as luminâncias por construção (os dois têm teto de luminância agora). Ou seja: os dois canais não-cromáticos abaixo deixaram de ser "reforço" e viraram a ÚNICA coisa que distingue risco de normal em escala de cinza — não remover. `packages/ui/src/contraste.test.ts` trava um piso de 1,2 entre os dois pra impedir que a aproximação vire colapso.
...
- ⚠️ **IMPORTANT 2 — `hover:underline` nunca aplica no Android real; o link do dono virava distinguível só por cor.** Tailwind v4 emite `hover:*` dentro de `@media (hover: hover)` (confirmado no CSS emitido) — em touch/mobile essa media query nunca casa. MEDIDO com `hasTouch: true, isMobile: true` (mesma emulação de um Android real): `textDecorationLine === 'none'` nos 5 links do `<nav>` (`App.tsx`) e no título da dívida (`DividasPage.tsx`, **o único jeito de abrir uma dívida**) nas 5 rotas. Cor sozinha (`rgb(58,191,248)` sobre `rgb(247,249,252)` = **1,99:1** de contraste, contra 17,46:1 do texto do corpo) não é suficiente affordance — a regra apagada de `base-interina.css` (`a { text-decoration: underline }`) era incondicional, sem depender de hover. Corrigido trocando `hover:underline` → `underline` nos dois lugares (`App.tsx#navLinkClassName`, `DividasPage.tsx`'s debt link). **MEDIDO depois do fix:** `textDecorationLine === 'underline'` nas 5 rotas, sem depender de `:hover`. **`--primary`'s contraste em si (1,99:1) não foi tocado** — é herança da Task 4 (o token em si, não desta migração), fora de escopo deste fix; registrado aqui, não corrigido.
  - ✅ **CORRIGIDO DEPOIS, na fatia de contraste dos tokens** — `--primary` do tema claro saiu de `198 93% 60%` para `198 93% 26%` (medido em Chrome real: **1,99 → 7,46:1** sobre o fundo da página). O `underline` incondicional continua sendo a affordance principal, e agora a cor também sustenta sozinha. Fonte única do fato, com a tabela completa e o gate que trava isso: `packages/ui/CLAUDE.md` § _Contraste dos tokens_.
