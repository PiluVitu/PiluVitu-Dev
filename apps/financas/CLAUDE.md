...
- **Testes:** `pnpm --filter @piluvitu/financas exec vitest run src/lib/session.test.ts` — matriz pura de `decidirAcesso`, `isRotaDeAuth`, e dois casos de integração HTTP via `app.request()` (sem cookie → 401; cookie genuinamente válido de e-mail fora da allowlist → 403, ver acima). `src/index.test.ts` cobre a montagem: `/api/health` público, `/api/accounts` sem cookie → 401, cookie com token/assinatura que não bate com nenhuma linha real → 401 (não lança — `getSession()` devolve `null` para um cookie bem-formado mas inexistente), `/api/auth/*` não passa pela nossa guarda (resposta não é `401` no nosso envelope).
