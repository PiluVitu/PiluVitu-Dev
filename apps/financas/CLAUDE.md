...
- **96 testes** (17 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos) + `pages/home.tsx` + `blocos/Bloco.tsx` + `blocos/BlocoComprometido.tsx` + `blocos/GraficoComprometido.tsx` + `lib/commitments.ts` (Task 6, incl. fix round 1, ver seção seguinte).
...
- **`src/pages/home.tsx`** — grid responsivo (`grid-cols-1` no celular, `md:grid-cols-2` a partir do MacBook) montando os blocos da home; nesta task só `BlocoComprometido`, as Tasks 7/8 adicionam os outros três no mesmo grid.
