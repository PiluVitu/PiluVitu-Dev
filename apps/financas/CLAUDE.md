...
- **104 testes** (19 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos) + `pages/home.tsx` + `blocos/Bloco.tsx` + `blocos/BlocoComprometido.tsx` + `blocos/GraficoComprometido.tsx` + `lib/commitments.ts` (Task 6, incl. fix round 1, ver seção seguinte) + `blocos/BlocoSaldos.tsx` + `blocos/BlocoDividas.tsx` (Task 7, ver seção _Task 7_ abaixo).
