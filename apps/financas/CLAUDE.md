...
- **75 testes** (12 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos).
