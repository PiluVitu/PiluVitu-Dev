...
## Better Auth — factory (`src/lib/auth.ts`) — Task 2, AINDA NÃO CONECTADA

O Access acima está sendo trocado por **Better Auth** com login social Google, porque Zero Trust exige cartão de crédito pra verificação e o dono não tem como. `src/lib/auth.ts` cria a instância; **`src/index.ts` continua no `requireAccess` até a Task 3** — este módulo não está montado em rota nenhuma ainda.
...
- **Camada 2 fica para a Task 3.** Better Auth não tem noção de allowlist fora do hook de criação — se uma sessão já existe para um e-mail desautorizado (ex.: allowlist trocada depois do login), ela é emitida e validada normalmente. `session.ts#decidirAcesso` (Task 3) é o segundo guard, sobre sessão já existente.
- **`nodejs_compat`** entrou em `wrangler.jsonc` por precaução, não correção de bug — o design doc afirma que não é obrigatório pra o Worker subir nesta `compatibility_date` (`node:crypto`/`node:async_hooks` já seriam módulos nativos aqui) e que só silencia 2 warnings de build sem pesar no bundle. **Isto NÃO foi (re)verificado nesta task.** `src/index.ts` não importa `src/lib/auth.ts` ainda (ver bullet do bundle abaixo) — então nenhum arquivo do grafo publicado importa `node:` nada, e os 2 warnings não apareceriam NEM COM NEM SEM a flag. Um `wrangler deploy --dry-run` limpo aqui não é evidência de que a flag faz alguma coisa; a verificação real fica pra Task 3, quando o import for ligado e `better-auth` (única fonte de `node:` neste pacote) entrar de fato no bundle.
...
- **`AuthBindings`** (`DB` + `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `ALLOWED_EMAIL`) hoje só existe em `.dev.vars` (gitignored, real). Promover pra `wrangler.jsonc`/secrets de produção é Task 3/5.
- ⚠️ **`ALLOWED_EMAIL` é UM e-mail só — NUNCA CSV, ao contrário de `ACCESS_ALLOWED_EMAILS`** (que era `.split(',')`, ver `access.ts`). `isAllowedEmail` compara STRING INTEIRA: um valor `"dono@exemplo.com,outro@exemplo.com"` não casa com nada, nem com o dono. Continua fail-closed (não é brecha), mas o sintoma é indistinguível de "o login do Google quebrou" — não carregar esse hábito do módulo anterior pra cá. Coberto por linha de teste dedicada em `auth.test.ts`.
- **Bundle ainda não reflete o Better Auth.** Nada em `src/index.ts` importa `src/lib/auth.ts` nesta task — o bundler faz tree-shaking do módulo inteiro. `wrangler deploy --dry-run` mede **27,66 KiB gzip / 114,58 KiB total** aqui, não os ~330 KiB que um spike com o import já ligado mediu; o número sobe quando a Task 3 conectar o handler em `/api/auth/*`. Segue MUITO abaixo do teto de 3 MB do Worker de qualquer forma.
