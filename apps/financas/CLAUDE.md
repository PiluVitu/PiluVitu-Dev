...
- **`nodejs_compat`** entrou em `wrangler.jsonc` por precaução, não correção de bug — MEDIDO (spike S3): não é obrigatório pra o Worker subir nesta `compatibility_date` (`node:crypto`/`node:async_hooks` já são módulos nativos aqui). Só silencia 2 warnings de build e não pesa no bundle gzipado (resolução de módulo nativo, não polyfill JS).
