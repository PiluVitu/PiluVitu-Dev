...
- **`src/blocos/BlocoSaldos.tsx`** — busca `GET /api/accounts` (a rota já esconde conta arquivada por padrão; o bloco não passa `?archived=1`) e separa o total em **PJ e PF**, nunca somados — é o motivo de `scope`/`is_business` existirem (ver CLAUDE.md raiz e `schema.sql`): misturar os dois faria a tela mentir sobre qual dinheiro é realmente do dono (a renda fixa PJ vs. o resto). Cada grupo mostra o total (`data-testid="total-PJ"`/`"total-PF"`) e a lista de contas com o próprio saldo (`data-testid="saldo-<id>"`), via `formatBRL`/`sumCents` de `@piluvitu/tools/money` — nunca float.
