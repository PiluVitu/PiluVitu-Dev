...
- **90 testes** (15 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos) + `pages/home.tsx` + `blocos/Bloco.tsx` + `blocos/BlocoComprometido.tsx` (Task 6, ver seção seguinte).
...
- **`src/blocos/GraficoComprometido.tsx` — módulo separado, carregado via `lazy()`/`Suspense`, nunca importado estaticamente.** É o único arquivo que importa `recharts` (**medido: +104,31 kB gzip** num chunk isolado — perto da estimativa de +110 kB do brief) — um `import` estático pesaria a primeira pintura da home inteira, mesmo pra quem nunca chega a ver o gráfico renderizado. `BlocoComprometido` faz `const GraficoComprometido = lazy(() => import('./GraficoComprometido'))` e só monta esse `<Suspense>` quando o relatório já chegou (`report !== null`) — antes disso `Bloco` já mostra o skeleton de "carregando". Prova em teste, não só na mecânica do React: `BlocoComprometido.test.tsx` afirma que `screen.queryByTestId('grafico-comprometido')` **não** está no DOM logo após o primeiro render síncrono, e só aparece depois de um `waitFor` — é o boundary do Suspense de verdade, não um `if (loading)` disfarçado.
  ...
  - **Sem `ResponsiveContainer`** — `width`/`height` fixos (`640×220`) dentro de um `<div className="overflow-x-auto">`. `ResponsiveContainer` depende de medir o container real via `ResizeObserver` (ausente no jsdom dos testes) e não renderiza filho nenhum sem essa medição — testar de forma determinística exigiria mockar `ResizeObserver` globalmente (não feito, pra não acoplar `src/test/setup.ts`, compartilhado por toda a suíte, a um único componente). Trade-off aceito: em telas mais estreitas que 640px (Android em retrato), o gráfico rola horizontalmente dentro do card em vez de espremer as barras — mobile-friendly sem depender de medição de container.
...
- **Bundle, medido (`vite build`, antes = Task 5 / depois = Task 6):**

  |                                    | antes                     | depois                                         |
  | ---------------------------------- | ------------------------- | ---------------------------------------------- |
  | JS principal                       | 239,37 kB / 76,18 kB gzip | 270,47 kB / 86,38 kB gzip                      |
  | Chunk `GraficoComprometido` (lazy) | —                         | 354,50 kB / 104,31 kB gzip                     |
  | CSS                                | 24,76 kB / 5,56 kB gzip   | 24,76 kB / 5,56 kB gzip (idêntico byte a byte) |

  CSS idêntico não é coincidência: as classes novas (`grid`, `gap-4`, `md:grid-cols-2`, `text-muted-foreground`, `overflow-x-auto`, ...) já estavam no CSS emitido antes desta task, porque `@source` varre `packages/ui/src` inteiro — cujos 14 componentes já usam esse mesmo conjunto de utilitários — não só o que a SPA de fato importa (ver bullet do gate, no topo desta seção). O crescimento do bundle **principal** (+10,2 kB gzip) vem de consumir `Card`/`CardHeader`/`CardTitle`/`CardContent`/`Skeleton` de `@piluvitu/ui` **pela primeira vez de verdade em código** — até a Task 5 nenhum dos 14 componentes era importado por nenhuma tela real (só as classes CSS eram geradas, sem o JS correspondente). `recharts` fica inteiro isolado no chunk lazy, fora do caminho crítico da primeira pintura.
