...
**Decisão de design, não coberta explicitamente pelo brief: `total_cents` é a soma COM SINAL de `amount_cents`, sem filtrar por `kind` da categoria nem por sinal.** Ao contrário de `committed_cents` em `commitments()` (que nega pra virar "quanto vou pagar", sempre positivo), aqui a rota devolve o total líquido por categoria tal como está no caixa — receita e despesa não são misturadas na prática porque nenhuma tela hoje lança receita e despesa sob a mesma categoria, e manter o sinal cru deixa o consumidor (Task 8) decidir a apresentação (pizza só de despesas, filtro por sinal, etc.) sem precisar de um segundo endpoint. Consequência: uma categoria só de receita sai com `total_cents` positivo; a UI que só quer "pra onde foi o dinheiro" filtra por `total_cents < 0` do lado dela.

**`LIMIT 500`** no `GROUP BY` — mesmo teto de `listTransactions()` — protege contra o caso patológico de uma quantidade grande de categorias distintas no mês; na prática o módulo semeia só 7 categorias e é single-user, então nunca é exercitado, mas fica pela mesma razão de `listTransactions`: no D1 "rows read" conta linha escaneada, nenhuma query fica sem teto. `total_cents` (o grande total) é somado em JS a partir das linhas devolvidas — correto enquanto o nº de categorias distintas no mês não passar do `LIMIT`.
...
Testes: `pnpm --filter @piluvitu/financas exec vitest run src/domain/reports.test.ts src/routes/reports.test.ts` — cobre soma por categoria, bucket "Sem categoria", mês vazio (`rows: []`, `total_cents: 0`), exclusão de perna de transferência e de filha de rateio (cada uma provada por um cenário que uma query sem o filtro faria falhar, não só por um valor que coincidentemente bate), a distinção `purchase_date`/`bill_competence` nas duas direções, e `competence` ausente/malformada/mês inexistente (`2026-13`) rejeitando com `RangeError`/`400 invalid_query`.
