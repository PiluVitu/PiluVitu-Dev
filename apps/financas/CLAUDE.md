...
- **114 testes** (20 arquivos) cobrindo as 5 telas + `api.ts` + `App.tsx` (roteamento por hash + Gate autenticado) + `lib/dates.ts` + `Gate.tsx` + `lib/theme.ts` (Task 4, 10 casos) + `base-interina.test.ts` (Task 4 fix round 1, 2 casos) + `pages/home.tsx` + `blocos/Bloco.tsx` + `blocos/BlocoComprometido.tsx` + `blocos/GraficoComprometido.tsx` (incl. `GraficoCategorias`, Task 8) + `lib/commitments.ts` (Task 6, incl. fix round 1, ver seção seguinte) + `blocos/BlocoSaldos.tsx` + `blocos/BlocoDividas.tsx` (Task 7, ver seção _Task 7_ acima) + `blocos/BlocoCategorias.tsx` (Task 8, ver seção _Task 8_ abaixo).
...
Esperado no `list` (no momento em que este parágrafo foi escrito): `0001_financas_init.sql` já aplicada; `0002_better_auth.sql`, `0003_account_provider_idx.sql` e `0004_tx_purchase_date_expense_idx.sql` pendentes — a última é o índice parcial que sustenta `GET /api/reports/by-category` (fix round 1 da Task 5, ver seção _Migrations_ → `0004_tx_purchase_date_expense_idx.sql` e _Relatório por categoria_ mais abaixo), aplicada só localmente até este ponto. Depois do `apply`, todas aparecem aplicadas. Sem down migration — se o schema sair errado, a correção é uma migration nova (`0005_*.sql`), nunca editar uma já rodada com `--remote`. Índice no D1 também não é alterável, só dropado (irreversível) e recriado — `0003` e `0004` já são essa decisão tomada conscientemente (ver seção _Migrations_ acima).
