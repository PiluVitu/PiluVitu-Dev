...
> O **backend** da votação (store, schema, auth, sorteio, TMDb, backup, endpoints) foi reimplementado no **ramielle** (Cloudflare Worker, Hono + D1) — ver `apps/ramielle/CLAUDE.md`. Este app (`apps/web`) foi repontado pro ramielle na fatia ④ do cutover, Task 2 (`.superpowers/sdd/2026-08-12-ramielle-fatia4-cutover/`): dos 20 caminhos chamados, 14 batem byte a byte com o Go e 1 (`GET /auth/google/login`) não existe — trocado por login via Better Auth (ver bullet do API client abaixo). ⚠️ **O commit da Task 2 não reponta produção sozinho** — `NEXT_PUBLIC_API_URL` está inlinado no bundle da Vercel; só passa a valer depois de merge + redeploy (runbook da T6). Até lá, `apps/api/CLAUDE.md` continua sendo o backend real de produção. Aqui é só o frontend.
...
- **API client:** `apps/web/lib/votacao/api-client.ts` faz fetch com `credentials: 'include'` contra `NEXT_PUBLIC_API_URL` (default `http://localhost:8080`). **Login (cutover T2):** `startGoogleLogin()` substituiu a antiga navegação top-level pro `GET /auth/google/login` (rota que não existe no ramielle — cai no catch-all, 404). O fluxo novo é `POST /api/auth/sign-in/social` (Better Auth) via `fetch` puro — **zero dependência nova**, `better-auth` continua não sendo dependência de `apps/web` (é dep. só de `apps/financas/web`); a lib inteira não vale a pena só pra montar uma URL. A resposta é `{url, redirect}` (verificado contra o pacote `better-auth@1.6.25` instalado) e quem navega é o cliente (`window.location.href = url`), não um `<a href>` estático. `callbackURL`/`errorCallbackURL` são **sempre absolutas** (`window.location.href`, a página atual) — diferente do padrão do finanças (`apps/financas/web/src/Gate.tsx`, path relativo), que só funciona lá porque Worker+SPA dividem a mesma origem; aqui `apps/web` (`piluvitu.com.br`) e o ramielle (`ramielle.piluvitu.com.br`) são origens diferentes, e um path relativo resolveria contra a origem ERRADA (quem processa o redirect OAuth é o ramielle). Falha de rede/HTTP lança `Error` tratável (nunca falha em silêncio) — `<LoginButton>` e `<AdminLoginScreen>` capturam e mostram a mensagem (toast / `role="alert"` inline, respectivamente). Trava de regressão: `lib/votacao/api-client.test.ts` (`describe('startGoogleLogin', …)`, inclui o teste que prova que a URL nunca contém `/auth/google/login`).
...
- **E2E mocks (`votacao.e2e.ts`):** `page.route()` usa globs host-agnósticos (`**/auth/me`, `**/votacao/sessions`, …) pra casar com qualquer `NEXT_PUBLIC_API_URL`, e os corpos mockados seguem o envelope (`envelope(data, notes)` / `errorEnvelope(code, msg)`). Não hardcodar `localhost:8080` — quebrava quando a porta da API mudava. Desde o cutover T2, `<LoginButton>` é um `<button>` (não mais um `<a href>`) — o teste `'shows login button when anonymous'` usa `getByRole('button', ...)`.
...
- **API client:** `lib/admin/atelier/api.ts` — chama `atelierBase` (`NEXT_PUBLIC_ATELIER_URL`, default `http://localhost:8080`) com `credentials: 'include'` e desembrulha o envelope `{ok,data,notifications}` via `ApiError` (tipo reaproveitado de `@/lib/votacao/api-client`). Metadados de plataforma (labels, charLimit) em `lib/admin/atelier/platform-meta.ts`.
  ...
  ⚠️ **`atelierBase` é DELIBERADAMENTE separada de `apiBase` (a base da votação, `NEXT_PUBLIC_API_URL`) — não fundir de novo.** Fatia ④ do cutover ramielle (2026-08-12): a Task 2 repontou o código da votação pro ramielle (`apiBase`/`startGoogleLogin` — ver bullet do API client em _Votação de Filmes_ acima; produção só reponta de fato depois do merge+redeploy da T6). ⚠️ **Correção (revisão final da fatia de distribuição, `.superpowers/sdd/2026-08-13-ramielle-distribuicao/`): o ramielle JÁ TEM as 5 rotas do Atelier** (`/admin/llm/proofread`, `/admin/llm/refine`, `/admin/distribution/proposals`, `/admin/distribution/{slug}`, `/admin/distribution/{slug}/publish` — ver `apps/ramielle/CLAUDE.md`, §§ _`lib/promeia.ts` e `/admin/llm/*`_ e _Distribuição de artigo_); elas respondem `503` (código `promeia_disabled`/`distribution_unavailable`) até os secrets serem cadastrados, não `404`. `NEXT_PUBLIC_ATELIER_URL` continuar apontando pra Go local é decisão de cutover ainda NÃO tomada pelo dono, não ausência de rota no ramielle. MEDIDO: se `atelierBase` voltar a derivar de `apiBase`, o card "Distribuição" (`components/admin/posts/distribution-panel.tsx`) passa a renderizar **vazio, sem nenhum sinal de erro**, em todo post que já tinha distribuição salva — `useDistribution(slug, !!slug)` dispara sozinho no mount (`enabled` verdadeiro), o `QueryClient` é `new QueryClient()` cru (sem `defaultOptions`, `retry` default = 3 ⇒ 4 requisições falhas por post aberto), e `targets = localTargets ?? existing.data?.targets ?? []` engole o erro (nada no componente lê `isError`). Trava de regressão: `lib/admin/atelier/api.test.ts` (`describe('atelierBase', …)`).
...
- `NEXT_PUBLIC_API_URL` — base URL da API de votação/admin consumida pelo front (default `http://localhost:8080`). **Desde a Task 2 da fatia ④ do cutover ramielle**, o valor de produção passa a ser o ramielle (`https://ramielle.piluvitu.com.br`, comentado em `.env.example`), não mais a Go — mas o commit da Task 2 sozinho **não reponta produção**: `NEXT_PUBLIC_API_URL` é inlinada no bundle da Vercel em build time, só passa a valer depois de cadastrar o valor real lá + redeploy (runbook da T6).
- `NEXT_PUBLIC_ATELIER_URL` — base URL do Atelier (revisão de artigo + distribuição, `/admin/posts`), **deliberadamente separada** de `NEXT_PUBLIC_API_URL` (default também `http://localhost:8080`, mesma Go local por enquanto — separação de cutover ainda não decidida pelo dono, não ausência de rota: o ramielle já tem as rotas do Atelier, ver bullet `atelierBase` acima). Ver seção _Atelier_ abaixo para o porquê. ⚠️ **O default `http://localhost:8080` é só pra dev local — NÃO cadastre esse valor na Vercel.** Em produção, sem essa variável explicitamente setada com a URL real do túnel da Go, o navegador do admin tenta `localhost:8080` (a própria máquina dele) e o card "Distribuição" fica vazio sem nenhum erro visível. Precisa entrar na Vercel com a URL real ao repontar `NEXT_PUBLIC_API_URL` pro ramielle — não basta copiar o `.env.example` literal.
