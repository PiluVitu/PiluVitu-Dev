...
- `NEXT_PUBLIC_ATELIER_URL` — base URL do Atelier (revisão de artigo + distribuição, `/admin/posts`), **deliberadamente separada** de `NEXT_PUBLIC_API_URL` (default também `http://localhost:8080`, mesma Go local por enquanto). Ver seção _Atelier_ abaixo para o porquê. ⚠️ **O default `http://localhost:8080` é só pra dev local — NÃO cadastre esse valor na Vercel.** Em produção, sem essa variável explicitamente setada com a URL real do túnel da Go, o navegador do admin tenta `localhost:8080` (a própria máquina dele) e o card "Distribuição" fica vazio sem nenhum erro visível. Precisa entrar na Vercel com a URL real ao repontar `NEXT_PUBLIC_API_URL` pro ramielle (Task 2 desta fatia) — não basta copiar o `.env.example` literal.
