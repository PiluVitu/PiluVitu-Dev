...
func TestRequestIDMatchesBetweenHeaderAndLog(t *testing.T) {
	var buf bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&buf, nil))

	h := middleware.RequestID(logging.Middleware(logger)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		logging.FromContext(r.Context()).Info("hi")
	})))
	...
	if entry["request_id"] != headerID {
		t.Fatalf("request_id mismatch: header=%q log=%v", headerID, entry["request_id"])
	}
}
