...
- **CLI:** cobra — `piluvitu <tool> <subcommand>` (e.g., `piluvitu cpf validate "123"`). Único consumidor de `internal/tools/` que sobrou depois do cutover acima — por isso o pacote **ficou**, só os handlers HTTP saíram.
- **Layer rules:** `internal/tools/` is pure Go (no HTTP, no cobra) — hoje linkado só pelo `cmd/cli`, não mais por nenhum handler HTTP; `internal/handlers/` (subpacotes `admin`, `distribution`, `llm`, `votacao`) delega pro domínio delas, não pra `internal/tools`; `internal/votacao/` é o pacote de domínio (Store + entidades); `cmd/` only parses args
