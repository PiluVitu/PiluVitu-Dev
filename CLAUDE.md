...
> | `apps/ramielle`  | `apps/ramielle/CLAUDE.md`  | Worker Cloudflare (Hono + D1), substituindo a API Go fatia a fatia: votação de filmes, auth Google (Better Auth, votação **LIVRE** — oposto do finanças), CORS, deploy `wrangler` |
...
- **`apps/ramielle`** — **Cloudflare Worker** (Hono + D1 SQLite), substituindo a API Go (`apps/api`) fatia a fatia — hoje: votação de filmes com login Google (**Better Auth**, mesma lib do finanças, mas votação **LIVRE**: sem allowlist de acesso, oposto do finanças que é fail-closed de usuário único). Vai morar em `api.piluvitu.com.br`; CORS explícito com credenciais porque `apps/web` mora em outra origem (`piluvitu.com.br`). Testes com `@cloudflare/vitest-pool-workers`. Nada em produção depende dele ainda — `apps/web` segue na Go. → detalhes em `apps/ramielle/CLAUDE.md`.
