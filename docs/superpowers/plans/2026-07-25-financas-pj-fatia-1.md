...
const erro = (await estouro.json()) as Envelope<null>
expect(erro.ok).toBe(false)
expect(erro.data).toBeNull()
expect(erro.notifications[0].code).toBe('over_allocation')
expect(erro.notifications[0].type).toBe('error')

    // nada persistiu: o batch inteiro reverteu
    const detalhe = await routes.request(`/debts/${debtId}`, {}, { DB: env.DB })
    expect(detalhe.status).toBe(200)
    const detalheBody = (await detalhe.json()) as Envelope<{
      items: Array<{ allocated_cents: number; remaining_cents: number }>
      payments: unknown[]
    }>
    expect(detalheBody.data?.items[0].allocated_cents).toBe(0)
    expect(detalheBody.data?.items[0].remaining_cents).toBe(100000)
    expect(detalheBody.data?.payments).toHaveLength(0)

})

it('GET /debts lista com total, pago e restante', async () => {
const payeeId = newId()
await env.DB.prepare(
`INSERT INTO payees (id, name, norm_name, kind, created_at) VALUES (?, 'Pai', 'PAI', 'person', ?)`,
)
.bind(payeeId, nowIsoUtc())
.run()

    const criada = await post('/debts', {
      payee_id: payeeId,
      direction: 'i_owe',
      title: 'Pai',
      opened_at: '2026-02-20',
    })
    const debtId = ((await criada.json()) as Envelope<{ id: string }>).data?.id as string

    const item = await post(`/debts/${debtId}/items`, {
      description: 'Steam Deck OLED 1TB',
      amount_cents: 280000,
      incurred_on: '2026-02-20',
    })
    const itemId = ((await item.json()) as Envelope<{ id: string }>).data?.id as string

    const pago = await post(`/debts/${debtId}/payments`, {
      paid_on: '2026-07-05',
      amount_cents: 144000,
      kind: 'offset',
      allocations: [{ item_id: itemId, amount_cents: 144000 }],
    })
    expect(pago.status).toBe(201)

    const lista = await routes.request('/debts?status=open', {}, { DB: env.DB })
    expect(lista.status).toBe(200)
    const listaBody = (await lista.json()) as Envelope<
      Array<{ total_cents: number; paid_cents: number; remaining_cents: number }>
    >
    expect(listaBody.ok).toBe(true)
    expect(listaBody.data?.[0].total_cents).toBe(280000)
    expect(listaBody.data?.[0].paid_cents).toBe(144000)
    expect(listaBody.data?.[0].remaining_cents).toBe(136000)

})

it('corpo que não é JSON devolve 400 invalid_json', async () => {
...
- [ ] **Step 23: Rodar o teste de rota e confirmar que passa**

Run: `pnpm --filter @piluvitu/financas exec vitest run src/routes/debts.test.ts`
Esperado: PASS — `3 passed`. (A rota já foi escrita no Step 21; este ciclo é RED só se o arquivo de rota tiver ficado com erro — se falhar com `Failed to resolve import "./debts"`, o Step 21 não foi salvo.)

- [ ] **Step 24: Montar a rota no app**

Em `apps/financas/src/index.ts`, ao lado das outras montagens de rota, adicionar o import e o `route`:
...
- [ ] **Step 25: Documentar o modelo de dívida no CLAUDE.md do workspace**

Acrescentar em `apps/financas/CLAUDE.md`:

```md
## Dívidas (`src/domain/debts.ts`)

**Estoque × fluxo.** `debt_items` é dimensão patrimonial e **nunca** gera lançamento;
`debt_payments` é fluxo e gera **exatamente uma** transaction, elo 1:1 garantido por
`uq_debt_payments_tx`. Os dois nunca se somam — a dupla contagem é estruturalmente
impossível, não é regra de relatório.

`payDebt` faz **duas** chamadas ao D1: uma de leitura (dívida + categoria de quitação,
em 1 round-trip) e **um único `db.batch()` de escrita** com:

1. `INSERT OR IGNORE` da categoria `kind='debt_settlement'` (só se ainda não existir)
2. `INSERT transactions` — apenas quando `kind='cash'`
3. `INSERT debt_payments`
4. N × `INSERT debt_payment_allocations`
5. `UPDATE debts SET status='settled'` quando não sobra item em aberto

**Regras que o código impõe:**

- `kind='cash'` exige `account_id`; `offset` e `forgiven` não movem caixa e **rejeitam**
  `account_id`.
- A **direção** decide o **sinal** da transaction (`i_owe` → negativo, `owed_to_me` →
  positivo), **nunca** a categoria: receber de quem devia é entrada de caixa mas não é
  receita — classificar como `income` inflaria o faturamento e distorceria o DAS.
- O fechamento automático usa `EXISTS (debt_items)` + `NOT EXISTS (v_debt_item_balance
... is_settled = 0)`, para que dívida sem item não seja fechada por engano.

**Tetos vivem no banco.** `trg_alloc_item_teto` e `trg_alloc_pagamento_teto` fazem
`RAISE(ABORT)`; o D1 lança com `SQLITE_CONSTRAINT_TRIGGER` e o `batch()` reverte a
sequência inteira. A aplicação só traduz o erro para `OverAllocationError` (HTTP 409,
code `over_allocation`). **Não existe batch compensatório nem inspeção de
`meta.changes`** — o padrão antigo do spec foi eliminado pela medição de 2026-07-25.
```

- [ ] **Step 26: Rodar a suíte inteira, prettier e lint**

Run: `pnpm --filter @piluvitu/financas exec vitest run && pnpm prettier:fix && pnpm lint`
Esperado: todos os testes do workspace verdes (incluindo os 5 de `installments.test.ts` da Task 8), prettier sem diff pendente, ESLint sem erro.

- [ ] **Step 27: Rodar o `make test` da raiz para garantir que nada quebrou nos outros workspaces**

Run: `make test`
Esperado: `pnpm -r test` + `go test` verdes.
...
```bash
git add apps/financas && git commit -m "feat(financas): dividas com itens e alocacao de pagamento por item

debt_items e estoque (dimensao patrimonial) e nunca gera lancamento;
debt_payments e fluxo e gera exatamente uma transaction, elo 1:1. payDebt roda
um unico batch de escrita: categoria de quitacao + transaction (so em cash) +
pagamento + N alocacoes + fechamento da divida.

Os tetos I1/I2 vivem no banco (trg_alloc_pagamento_teto / trg_alloc_item_teto);
o SQLITE_CONSTRAINT_TRIGGER e traduzido em OverAllocationError e o batch reverte
inteiro, sem batch compensatorio.

Testes: cenario real do dono (Steam Deck 2.800 + MacBook 4.500 com 5.940 pagos
=> MacBook quitado e Steam Deck com 1.360 restantes), superalocacao sem rastro,
teto exato, 1x no caixa / 1x na divida / 0x na despesa, offset e forgiven sem
transaction, cash sem account_id rejeitado, owed_to_me nao vira income e
fechamento automatico no ultimo item."
```
